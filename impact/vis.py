...
class Plots(object):
    ...
    def draw_results(self, model, dataset, conffile='config/datafit.json'):
        ...
        canvas.Update()
        canvas.SaveAs('impact-analysis-{0}-{1}.eps'.format(dataset.energy,model.name))
    ...
    def fit(self,
		...
		hax = ROOT.TH1D("hax-{0}".format(model.name),"",10,data.GetXaxis().GetXmin(),data.GetXaxis().GetXmax())
		...
		hax.SetAxisRange(-5.23,5.23,"Y") #ratio.GetMinimum(data.GetXaxis().GetXmin() + 1,data.GetXaxis().GetXmax()) - 0.52,ratio.GetMaximum(data.GetXaxis().GetXmin() + 1,data.GetXaxis().GetXmax()) + 0.52
		hax.SetStats(False)
		hax.Draw()
